...
router.post("/add", function (req, res) {
  ...
  const url = "https://openlibrary.org/search.json?title=" + encodeURIComponent(searchTitle);
  ...
  fetch(url)
    ...
    .then(function (data) {
      ...
      if (doc) {
        ...
        if (doc.author_name && doc.author_name.length > 0) author = doc.author_name[0];
        if (doc.first_publish_year) year = String(doc.first_publish_year);
